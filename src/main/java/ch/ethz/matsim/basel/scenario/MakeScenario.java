...
public class MakeScenario {
	...
	static public void main(String args[]) throws Exception {
		...
		new PopulationReader(scenario).readFile(OUTPUT+"/population.xml.gz");
		new ObjectAttributesXmlReader(scenario.getPopulation().getPersonAttributes())
				.readFile(OUTPUT+"/population_attributes.xml.gz");
		new MatsimFacilitiesReader(scenario).readFile(OUTPUT+"/facilities.xml.gz");
		new MatsimNetworkReader(scenario.getNetwork()).readFile(INPUT+"/FinalNetwork2.xml.gz");
...
//		new TrafficCountPlanSelector(scenario.getNetwork(), countItems, scenarioScale, 0.01, numberOfThreads, "counts_locchoice.txt", 20).run(scenario.getPopulation());
		...
		new PopulationWriter(scenario.getPopulation()).write(OUTPUT+"/output_population.xml.gz");
		new ObjectAttributesXmlWriter(scenario.getPopulation().getPersonAttributes())
				.writeFile(OUTPUT+"/output_population_attributes.xml.gz");
		new FacilitiesWriter(scenario.getActivityFacilities()).write(OUTPUT+"/output_facilities.xml.gz");
		new NetworkWriter(scenario.getNetwork()).write(OUTPUT+"/output_network.xml.gz");
	}
}
